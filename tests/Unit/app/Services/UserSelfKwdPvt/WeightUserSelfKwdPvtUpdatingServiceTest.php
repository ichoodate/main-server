...
class WeightUserSelfKwdPvtUpdatingServiceTest extends _TestCase {
    ...
    public function testCallbackAuthUser()
    {
        $this->when(function ($proxy, $serv) {
            ...
            $weight          = $this->mMock();
            $weightQuery     = $this->mMock();
            $weightBaseQuery = $this->mMock();
            $kwdPvt          = $this->mMock();
            $kwdPvtQuery     = $this->mMock();
            $kwdPvtBaseQuery = $this->mMock();

            InstanceMocker::add(Weight::class, $weight);
            ModelMocker::query($weight, $weightQuery);
            QueryMocker::qSelect($weightQuery, Weight::ID);
            QueryMocker::getQuery($weightQuery, $weightBaseQuery);
            ...
            QueryMocker::qWhereIn($kwdPvtQuery, UserSelfKwdPvt::KEYWORD_ID, $weightBaseQuery);
